$fallbackDir = Join-Path (Get-Location) "7z"
$fallback7zr = Join-Path $fallbackDir "7zr.exe"
$fallback7za = Join-Path $fallbackDir "7za.exe"
$useragent = "mpv-win-updater"

function Get-7z {
    $7z_command = Get-Command -CommandType Application -ErrorAction Ignore 7z.exe, 7za.exe | Select-Object -Last 1
    if ($7z_command) {
    ...
    if ($7zdir) {
        if (Test-Path (Join-Path $7zdir "7z.exe")) {
            return Join-Path $7zdir "7z.exe"
        }
        if (Test-Path (Join-Path $7zdir "7za.exe")) {
            return Join-Path $7zdir "7za.exe"
        }
    }

    if (Test-Path $fallback7za) {
        return $fallback7za
    }
    if (Test-Path $fallback7zr) {
        return $fallback7zr
    }
...
function Download-7za {
    Write-Host "Descargando 7za.exe" -ForegroundColor Green
    $page = Invoke-WebRequest -Uri "https://www.7-zip.org/download.html" -UserAgent $useragent
    $match = $page.Links | Where-Object { $_.href -match "7z.*\-extra.7z$" } | Select-Object -First 1
	
    if (-not $match) {
        Write-Host "No se pudo encontrar el enlace a 7za.7z en la pagina." -ForegroundColor Red
        return
    }
	
    $url = "https://www.7-zip.org/" + $match.href
    $archivePath = Join-Path $fallbackDir "7za.7z"
    Invoke-WebRequest -Uri $url -UserAgent $useragent -OutFile $archivePath
    & $fallback7zr x $archivePath -o"$fallbackDir" -y
    Remove-Item $archivePath -Force
}

function Check-7z {
    if (-not (Get-7z)) {
        $null = New-Item -ItemType Directory -Force $fallbackDir
        Write-Host "Descargando 7zr.exe" -ForegroundColor Green
        Invoke-WebRequest -Uri "https://www.7-zip.org/a/7zr.exe" -UserAgent $useragent -OutFile $fallback7zr
        Download-7za
    }
    else {
        Write-Host "7z ya existe. Saltando descarga. " -ForegroundColor Green
    }
}

function Check-PowershellVersion {
    $version = $PSVersionTable.PSVersion.Major
    Write-Host "Verificando version de Windows PowerShell -- $version" -ForegroundColor Green
    if ($version -le 2)
    {
        Write-Host "La version $version de Windows PowerShell no esta soportada. Actualice su Windows PowerShell. " -ForegroundColor Red
        throw
    }
}
...
function Download-Ytplugin ($plugin, $version) {
    ...
    switch -wildcard ($plugin) {
        "yt-dlp*" {
            Write-Host "Descargando $plugin ($version) " -ForegroundColor Green
            $32bit = ""
...
function Extract-Archive {
    ...
    $7z = Get-7z
    Write-Host "Extrayendo " $file -ForegroundColor Green

    if ($outputDir) {
        if (-not (Test-Path $outputDir)) {
            Write-Host "La carpeta de salida '$outputDir' no existe. Creandola... " -ForegroundColor Yellow
            New-Item -ItemType Directory -Path $outputDir -Force | Out-Null
        }

        & $7z x -y $file "-o$outputDir"
    } else {
        & $7z x -y $file
    }
}
...
function Get-Latest-Mpv($Arch, $channel) {
    ...
    switch -wildcard ($channel) {
        ...
        "weekly" {
            ...
            Write-Host "Obteniendo la fuente RSS para mpv. " -ForegroundColor Green
...
function Get-Latest-Ytplugin ($plugin) {
    switch -wildcard ($plugin) {
        "yt-dlp*" {
            ...
            Write-Host "Obteniendo la fuente RSS para yt-dlp. " -ForegroundColor Green
        ...
        "youtube-dl" {
            ...
            Write-Host "Obteniendo la fuente RSS para youtube-dl. " -ForegroundColor Green
...
function Check-Arch($arch) {
    ...
    if ($doc.settings.arch -eq "unset") {
        if ($arch -eq "i686") {
            ...
            $result = Read-KeyOrTimeout "Elija la variante de 64 bits: x64 o x64-v3 (CPU con soporte AVX2) [1=x86_64 / 2=x86_64-v3 (pred=1)" "D1"
            Write-Host ""
            if ($result -eq 'D1') {
                $get_arch = "x86_64"
            }
            elseif ($result -eq 'D2') {
                ...
                throw "Por favor introduzca una tecla valida "
            }
        }
...
function Check-GetFFmpeg() {
    ...
    if ($doc.settings.getffmpeg -eq "unset") {
        Write-Host "FFmpeg no existe. Se procede a descargar. " -ForegroundColor Green -NoNewline
        $get_ffmpeg = "true"
...
function Upgrade-Mpv {
    ...
    if (Check-Mpv) {
        ...
        if ($localgit -match $remotegit)
        {
            ...
            {
                Write-Host "Ya estas usando la ultima version de mpv -- $remoteName . " -ForegroundColor Green
                $need_download = $false
            }
            else {
                Write-Host "Nueva version de mpv disponible. " -ForegroundColor Green
                $need_download = $true
            }
        }
        else {
        ...
        Write-Host "mpv no existe. Se procede a descargar. " -ForegroundColor Green -NoNewline
		$need_download = $true
		if (Test-Path (Join-Path $env:windir "SysWow64")) {
			Write-Host "Detectando: sistema de 64 bits" -ForegroundColor Green
			$original_arch = "x86_64"
		}
		else {
			Write-Host "Detectando: sistema de 32 bits" -ForegroundColor Green
			$original_arch = "i686"
		}
		$channel = Check-ChannelRelease
...
function Upgrade-Ytplugin {
    ...
    if ($yt) {
        ...
        if ((& $yt --version) -match ($latest_release)) {
            Write-Host "Ya estas usando la ultima version. " (Get-Item $yt).BaseName "-- $latest_release" -ForegroundColor Green
        }
        else {
            Write-Host "Nueva compilacion" (Get-Item $yt).BaseName "disponible. " -ForegroundColor Green
            & $yt --update
        }
    }
    else {
        Write-Host "yt-dlp o youtube-dl no existen. " -ForegroundColor Green -NoNewline
...
function Upgrade-FFmpeg {
    ...
    if ($ffmpeg_exist) {
        ...
        if ($local_git -match $remote_git) {
            Write-Host "Ya estas usando la ultima compilacion de ffmpeg -- $remote_name" -ForegroundColor Green
            $need_download = $false
        }
        else {
            Write-Host "Nueva compilacion de ffmpeg disponible. " -ForegroundColor Green
            $need_download = $true
        }
    }
    else {
...
function Get-Latest-Syncplay {
    $api_gh = "https://api.github.com/repos/Syncplay/syncplay/releases/latest"
    $json = Invoke-WebRequest $api_gh -MaximumRedirection 0 -ErrorAction Ignore -UseBasicParsing | ConvertFrom-Json
    $filename = $json.assets | where { $_.name -Match "Portable.zip" } | Select-Object -ExpandProperty name
    $download_link = $json.assets | where { $_.name -Match "Portable.zip" } | Select-Object -ExpandProperty browser_download_url
    if ($filename -is [array]) {
        return $filename[0], $download_link[0]
    }
    else {
...
perplayerarguments = {'C:\\Program Files\\mpv\\mpv.exe': ''}
...
mediasearchdirectories = ['C:\\Stream\\']
...
trusteddomains = ['youtube.com', 'youtu.be', 'icedrive.io', '*.icedrive.io']
...
"@ | Set-Content "C:\Program Files\Syncplay\syncplay.ini" -Encoding UTF8
}

function borrar-7za {
	$archivoExcluido = "7zr.exe"
	$directorioActual = Get-Location
	$carpeta = Join-Path -Path $directorioActual -ChildPath "7z"
	if (Test-Path $carpeta) {
		$archivos = Get-ChildItem -Path $carpeta | Where-Object { $_.Name -ne $archivoExcluido }
		foreach ($archivo in $archivos) {
			Remove-Item -Path $archivo.FullName -Force -Recurse
		}
	} else {
		Write-Host "Carpeta 7z no encontrada. Saltando. "
	}
}

function Install-Syncplay {
	
	if (-not (Test-Path "C:\Stream")) { New-Item -Path "C:\Stream" -ItemType Directory | Out-Null }
	if (-not (Test-Path "C:\Program Files\Syncplay")) { New-Item -Path "C:\Program Files\Syncplay" -ItemType Directory | Out-Null }
	
	$useragent = "Mozilla/5.0"
	
	# Instala VC_redist.x86.exe
	Download-Archive VC_redist.x86.exe https://aka.ms/vs/17/release/vc_redist.x86.exe
	Start-Process -FilePath VC_redist.x86.exe -ArgumentList "/install", "/quiet", "/norestart" -Wait
	Remove-Item -Force VC_redist.x86.exe
	Write-Host "VC_redist.x86.exe instalado."
	
	# Instala Syncplay Portable
	$remoteName, $link = Get-Latest-Syncplay
	Download-Archive $remoteName $link
	Check-7z
	Extract-Archive $remoteName "C:\Program Files\Syncplay\"
	Remove-Item -Force $remoteName
	Remove-Item -Force "C:\Program Files\Syncplay\syncplay.ini"
	Create-ini
	$s=(New-Object -ComObject WScript.Shell).CreateShortcut("$env:ProgramData\Microsoft\Windows\Start Menu\Programs\actualizar mpv.lnk");$s.TargetPath="C:\Program Files\mpv\mpv-updater_admin.bat";$s.IconLocation="C:\Program Files\mpv\installer\mpv-icon.ico";$s.Save()
	$s=(New-Object -COM WScript.Shell).CreateShortcut("$env:ProgramData\Microsoft\Windows\Start Menu\Programs\Syncplay.lnk");$s.TargetPath="C:\Program Files\Syncplay\Syncplay.exe";$s.Save()
	borrar-7za
	$archivoExe = "C:\Program Files\Syncplay\Syncplay.exe"
	if (Test-Path $archivoExe) {
		Start-Process $archivoExe
	} else {
		Write-Host "No se encontro Syncplay.exe. Instale manualmente."
		Start-Process "https://github.com/Syncplay/syncplay/releases"
	}
}
...
#
if (Test-Admin) {
    Write-Host "Ejecutar script con privilegios de administrador" -ForegroundColor Yellow
}
else {
    Write-Host "Ejecutar script sin privilegios de administrador" -ForegroundColor Red
}
